...
import static com.zy.seckill.common.mapper.DataDictDynamicSqlSupport.*;
...
public interface DataDictMapper {
    ...
    default int insertSelective(DataDict record) {
        return insert(SqlBuilder.insert(record)
                ...
                .map(multiple).toPropertyWhenPresent("multiple", record::getMultiple)
                .map(selected).toPropertyWhenPresent("selected", record::getSelected)
                .map(mutex).toPropertyWhenPresent("mutex", record::getMutex)
    ...
    default QueryExpressionDSL<MyBatis3SelectModelAdapter<List<DataDict>>> selectByExample() {
        return SelectDSL.selectWithMapper(this::selectMany, id, name, value, alias,
                        orderNum, parentId, multiple, selected, mutex, status, createTime)
                .from(dataDict);
    }

    @Generated("org.mybatis.generator.api.MyBatisGenerator")
    default QueryExpressionDSL<MyBatis3SelectModelAdapter<DataDict>> selectByExampleOne() {
        return SelectDSL.selectWithMapper(this::selectOne, id, name, value, alias,
                        orderNum, parentId, multiple, selected, mutex, status, createTime)
                .from(dataDict);
    }
    ...
    default QueryExpressionDSL<MyBatis3SelectModelAdapter<List<DataDict>>> selectDistinctByExample() {
        return SelectDSL.selectDistinctWithMapper(this::selectMany, id, name, value, alias,
                        orderNum, parentId, multiple, selected, mutex, status, createTime)
                .from(dataDict);
    }
